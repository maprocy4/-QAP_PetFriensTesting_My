...
class PetFriends:
    ...
    def create_pet_simple(self, auth_key: json, name: str, animal_type: str, age: int) -> json:
        """Метод отправляет (постит) на сервер данные о "простом" добавляемом питомце (без фотографии)
    ...
    def set_photo(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        ...
        res = requests.post(self.base_url + "/api/pets/set_photo/" + pet_id, headers=headers, data="", files=file)
        status = res.status_code
